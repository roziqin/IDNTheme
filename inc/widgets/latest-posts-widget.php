...
if ( ! class_exists( 'Architectonic_Latest_Post' ) ) :

     
    class Architectonic_Latest_Post extends WP_Widget {
        /**
        ...
        public function __construct() {
            $tp_widget_popular_post = array(
                ...
                'description' => esc_html__( 'Retrive latest posts.', 'architectonic' ),
            );
            parent::__construct( 'architectonic_latest_post', esc_html__( 'TP : Latest Posts', 'architectonic' ), $tp_widget_popular_post );
        }
                ...
                <li class="has-post-thumbnail clear">
                    ...
                    <a href="<?php the_permalink( $post->ID ); ?>">
                        ...
                        if ( has_post_thumbnail( $post->ID ) ) :
                            $image = get_the_post_thumbnail( $post->ID, $size = 'thumbnail', array( 'alt' => the_title_attribute( array( 'echo' => false, 'post' => $post->ID ) ) ) );
                            echo $image;
                        else :
        ...
        public function form( $instance ) {
            $tp_title      = isset( $instance['title'] ) ? ( $instance['title'] ) : esc_html__( 'Latest Posts', 'architectonic' );
            $tp_number     = isset( $instance['number'] ) ? absint( $instance['number'] ) : 3;
            ?>

            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title:', 'architectonic' ); ?></label>
            ...
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of posts to show:', 'architectonic' ); ?></label>
